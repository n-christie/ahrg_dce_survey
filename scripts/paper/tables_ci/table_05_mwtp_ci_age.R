# table_05_mwtp_ci_age.R
# Table 5 (CI version): MWTP with 95% confidence intervals — age interaction.
# Shows reference (Young, age < 72) and Old (age >= 72) MWTPs side by side.
# 95% CIs via delta method. No significance stars.
# Output: output/tables_ci/table_05_mwtp_ci_age.html
...
inter_group <- "Old"
ref_label   <- "Young (age < 72)"
inter_label <- "Old (age ≥ 72)"
...
tbl_html <- kbl(tab_df,
    format  = "html",
    escape  = FALSE,
    caption = "Table 5. MWTP by age group (SEK/month; 95% CI in brackets).",
    align   = c("l", "r", "r", "r", "r")) |>
  ...
  footnote(
    general = paste0(
      ...
      "Old = age ≥ 72 (above sample median). ",
      "Reference group: Young (age < 72)."
    ),
    general_title = "Note: "
  )

out_path <- here("output/tables_ci", "table_05_mwtp_ci_age.html")
writeLines(
...
message("Table 5 CI saved to ", out_path)

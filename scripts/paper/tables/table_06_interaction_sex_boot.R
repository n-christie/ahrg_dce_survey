# table_06_interaction_sex_boot.R
# Table 6: Mixed logit interaction regression — sex (Men vs Women as reference).
# Identical to table_06_interaction_sex.R but uses a non-parametric bootstrap for
...
# (for both base and Men groups) is computed. Percentile 95% CIs are reported
...
#
# Output: output/tables/table_06_interaction_sex_boot.html
#
...
# Create explicit attribute dummies and sex interaction terms (matches interaction_regs_owner_sex.R)
add_sex_interaction_vars <- function(df) {
...
df_owner_sex  <- add_sex_interaction_vars(df_owner)
df_renter_sex <- add_sex_interaction_vars(df_renter)

# Warm-start values from original sex interaction models
start_own  <- coef(readRDS(here("output/models", "mxl_sex_owner.rds")))
start_rent <- coef(readRDS(here("output/models", "mxl_sex_renter.rds")))

# ── Model parameter specification (must match mxl_sex_owner/renter) ───────────
...
main_pars  <- c("dist_green5km", "dist_green500m",
                ...
                "price_num")
inter_pars <- c("green5km_Men", "green500_Men",
...
inter_terms <- c("green5km_Men", "green500_Men",
                 "shops5km_Men", "shops500_Men",
                 "trans600_Men", "trans300_Men",
...
compute_group_mwtp <- function(model, scaler) {
  ...
  beta_pMen <- get_b("price_Men")

  out <- sapply(main_attrs, function(a) {
    beta_a    <- get_b(a)
    iterm     <- names(interaction_to_base)[interaction_to_base == a]
    beta_aMen <- if (length(iterm) && nzchar(iterm)) get_b(iterm) else 0

    A_base <- beta_a
    A_men  <- beta_a + beta_aMen
    C_base <- beta_p
    C_men  <- beta_p + beta_pMen

    c(
      base = if (any(is.na(c(A_base, C_base)))) NA_real_ else -(A_base / C_base) * scaler,
      men  = if (any(is.na(c(A_men,  C_men))))  NA_real_ else -(A_men  / C_men)  * scaler
    )
  })
  setNames(as.vector(out), paste0(rep(main_attrs, each = 2), c(".base", ".men")))
}
...
cache_own  <- file.path(boot_dir, "boot_mwtp_sex_owner.rds")
cache_rent <- file.path(boot_dir, "boot_mwtp_sex_renter.rds")
...
boot_own  <- run_bootstrap(df_owner_sex,  scaler_own,  cache_own,  seed = 2384, start_vals = start_own)
boot_rent <- run_bootstrap(df_renter_sex, scaler_rent, cache_rent, seed = 7651, start_vals = start_rent)
...
htmlreg(
  ...
  na.replace         = "\u2013",
  caption            = paste0(
    "Table 6. Interaction regression with sex (Men vs Women as reference). ",
    "MWTP CIs via non-parametric bootstrap (B = ", N_BOOT, ", percentile method)."
  ),
  caption.above = TRUE,
  file          = here("output/tables", "table_06_interaction_sex_boot.html")
)

message("Table 6 (bootstrap) saved to output/tables/table_06_interaction_sex_boot.html")
